...
static NSString *const meSegueName = @"@Me";
static NSString *const showMainUITabBarControllerSegueName = @"showMainUITabBarController";
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:showMainUITabBarControllerSegueName]) {
    ...
    [khController getItemAsync:userUrl
         ...
         if (error != nil) {
             ...
             if (![[self khUser] requiredFieldsMet]) {
                 ...
                 [[self khUser] setExternalId: [user objectID]];
                 [[self khUser] setExternalIdType: Facebook];
                 [[self khUser] setName: [user name]];
                 [[self khUser] setImage:[NSString stringWithFormat:
                                          [KHConfiguration getConfiguration: _facebookImageUrlConfigurationName],
                                          [user objectID]]];
                 [[self khUser] setEmail: [user objectForKey: _email]];
                 [self performSegueWithIdentifier:meSegueName sender:nil];
             }
             else
             {
                 [self performSegueWithIdentifier:showMainUITabBarControllerSegueName sender:nil];
             }
